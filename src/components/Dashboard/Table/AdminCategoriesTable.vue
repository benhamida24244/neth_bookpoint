  ...
  <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <thead class="bg-gray-50">
          <tr>
            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
              {{ translations.dashboard?.categories?.table?.hash }}
            </th>
            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
              {{ translations.dashboard?.categories?.table?.name }}
            </th>
            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
              {{ translations.dashboard?.categories?.table?.status }}
            </th>
            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
              {{ translations.dashboard?.categories?.table?.orders }}
            </th>
            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
              {{ translations.dashboard?.categories?.table?.books }}
            </th>
            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
              {{ translations.dashboard?.categories?.table?.createdAt }}
            </th>
            <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
              {{ translations.dashboard?.categories?.table?.action }}
            </th>
          </tr>
        </thead>
        <tbody class="divide-y divide-gray-200">
          ...
          >
              ...
              >
                {{
                  typeof category.status === 'number'
                    ? category.status === 1
                      ? translations.dashboard?.categories?.statusActive
                      : translations.dashboard?.categories?.statusInactive
                    : category.status
                }}
              </span>
            </td>
            ...
            <td class="px-6 py-4 whitespace-nowrap text-center">
              ...
              >
                <i class="far fa-eye"></i> {{ translations.dashboard?.categories?.table?.view }}
              </RouterLink>
            </td>
          </tr>
        </tbody>
    ...
    <div v-if="categories.length === 0" class="p-12 text-center">
      ...
      <h3 class="text-lg font-medium text-gray-900 mb-1">
        {{ translations.dashboard?.categories?.emptyHeader }}
      </h3>
      <p class="text-gray-500">{{ translations.dashboard?.categories?.emptySubtext }}</p>
    </div>
  </div>
</template>
