  ...
  >
    <div class="bg-white rounded-xl shadow-lg p-6 w-full max-w-lg relative transform transition-all" @click.stop>
      <h2 class="text-xl font-bold mb-4">
        {{
          isEditMode
            ? translations.dashboard?.editAuthorModal?.title
            : translations.dashboard?.addAuthorModal?.title
        }}
      </h2>
      ...
      <form @submit.prevent="handleSubmit">
        <div class="space-y-4">
          <div>
            <label for="AuthorName" class="block mb-2 font-medium"
              >{{ translations.dashboard?.addAuthorModal?.labels?.name }}
          ...
          <div>
            <label for="AuthorCountry" class="block mb-2 font-medium"
              >{{ translations.dashboard?.addAuthorModal?.labels?.country }}
              <span class="text-red-500">*</span></label
            >
            <input
              id="AuthorCountry"
          ...
          <div>
            <label for="AuthorDescription" class="block mb-2 font-medium">{{
              translations.dashboard?.addAuthorModal?.labels?.description
            }}</label>
            <textarea
          ...
          <div>
            <label for="AuthorImage" class="block mb-2 font-medium">{{
              translations.dashboard?.addAuthorModal?.labels?.image
            }}</label>
        ...
        <div class="flex justify-end gap-2 mt-6">
          <button
            ...
            :disabled="isLoading"
          >
            {{ translations.dashboard?.addAuthorModal?.cancel }}
          </button>
          <button
            type="submit"
          ...
          >
            ...
            <span v-else>{{
              isEditMode
                ? translations.dashboard?.editAuthorModal?.save
                : translations.dashboard?.addAuthorModal?.add
            }}</span>
          </button>
        </div>
      </form>
