...
<template>
  <div class="w-full min-h-screen bg-gray-50 py-8 px-4 sm:py-16">
    <div v-if="isLoading" class="text-center py-20">
      <div
        class="animate-spin rounded-full h-12 w-12 border-b-2 border-[var(--color-primary)] mx-auto"
      ></div>
      <p class="mt-4 text-gray-600">{{ t('singleCategoryPage.loading') }}</p>
    </div>
    ...
    <div v-else-if="currentCategory" class="max-w-7xl mx-auto">
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12">
        ...
        <div class="lg:col-span-2 flex flex-col justify-center space-y-6 relative">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-8"></div>
        </div>
      </div>
      ...
      <section class="max-w-7xl mx-auto px-4 py-12 lg:py-16">
        <header class="text-center mb-12">
          ...
          >
            {{ t('singleCategoryPage.booksIn') }} {{ currentCategory.name }}
          </h2>
          <div
