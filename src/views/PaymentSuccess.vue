...
onMounted(async () => {
  // تأكد من وجود customer و token
  if (!props.customer || !props.token) {
  ...
  try {
    const order = await ordersStore.fetchOrderByPaypalToken(props.customer, props.token)
    if (order) {
...
<template>
  <div class="min-h-screen bg-gray-50 flex items-center justify-center p-4 font-BonaRegular">
    ...
    <div v-else-if="currentOrder" class="max-w-2xl w-full bg-white rounded-2xl shadow-xl p-8 transition-all transform hover:scale-105 duration-300">
      ...
      <div class="bg-gray-100 rounded-xl p-6 mb-6">
        <h2 class="text-xl font-semibold text-gray-700 mb-4">{{ t('paymentSuccess.orderSummary') }}</h2>
        <div class="space-y-3">
          <div v-for="item in currentOrder.items" :key="item.id" class="flex items-center justify-between">
            <div class="flex items-center">
              <img :src="`${apiBaseUrl}${item.book.cover}`" :alt="item.book.title" class="w-16 h-16 rounded-lg mr-4 object-cover" />
              <div>
                <p class="font-medium text-gray-800">{{ item.book.title }}</p>
                <p class="text-sm text-gray-500">{{ t('paymentSuccess.quantity') }}{{ item.quantity }}</p>
              </div>
            </div>
      ...
      <div class="border-t pt-4 space-y-2 text-gray-700">
        ...
        <div class="flex justify-between">
          ...
          <span>${{ currentOrder.totals?.shipping?.toFixed(2) || '0.00' }}</span>
        </div>
        <div class="flex justify-between text-sm">
          <span>{{ t('paymentSuccess.vat') }}</span>
          <span>${{ currentOrder.totals?.vat?.toFixed(2) || '0.00' }}</span>
        </div>
        <div class="flex justify-between text-lg font-bold text-gray-900 border-t pt-2 mt-2">
          <span>{{ t('paymentSuccess.total') }}</span>
          <span class="text-green-600">${{ currentOrder.totals?.total?.toFixed(2) || subtotal.toFixed(2) }}</span>
        </div>
      </div>

      <div class="mt-8 text-center">
        <RouterLink to="/shop" class="bg-[var(--color-primary)] hover:bg-[var(--color-hover)] text-white font-bold py-3 px-6 rounded-lg transition-colors duration-300">
    ...
    <div v-else class="text-center">
      <h1 class="text-2xl font-bold text-gray-700">{{ error || t('paymentSuccess.noOrderDetails') }}</h1>
      <p class="text-gray-500 mt-2">{{ t('paymentSuccess.completePurchase') }}</p>
      <RouterLink to="/shop" class="mt-4 inline-block bg-[var(--color-primary)] hover:bg-[var(--color-hover)] text-white font-bold py-3 px-6 rounded-lg transition-colors duration-300">
