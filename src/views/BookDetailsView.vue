<template>
  <div class="bg-[#F3F4F6] py-8 font-BonaRegular min-h-screen">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div v-else-if="error || !book">
        <div class="text-center py-20 text-red-500">
            <h2 class="text-2xl font-bold mb-2">{{ t('bookdetails.bookNotFound') }}</h2>
            <p>{{ error }}</p>
        </div>
      </div>
      <div v-else>
        ...
        <nav class="text-sm mb-4 py-2 sticky top-0 z-10 bg-[#F3F4F6]">
          <ol class="list-none p-0 inline-flex">
            <li class="flex items-center">
              <RouterLink to="/" class="text-gray-500 hover:text-gray-700">{{ t('bookdetails.home') }}</RouterLink>
...
const { book, isLoading, error } = storeToRefs(booksStore);
...
onBeforeRouteUpdate(async (to, from, next) => {
  window.scrollTo(0, 0);
  await fetchBookData(Number(to.params.id));
  next();
});
