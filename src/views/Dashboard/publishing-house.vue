  ...
  <div class="w-full sm:px-8 lg:px-16 mt-8">
    ...
    <div v-if="isDeleteDialogOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div class="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
        <h3 class="text-lg font-bold text-gray-900">{{ translations.dashboard?.publishers?.deleteConfirm }}</h3>
        <p class="mt-2 text-sm text-gray-600">
          {{ translations.dashboard?.publishers?.deleteConfirmText }} "<strong>{{ publisherToDelete?.name }}</strong>"?
        </p>
        <div class="mt-6 flex justify-end gap-3">
          ...
          >
            {{ translations.dashboard?.publishers?.actions.cancel }}
          </button>
          <button
            @click="confirmDelete"
            class="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700"
          >
            {{ translations.dashboard?.publishers?.actions.delete }}
          </button>
        </div>
      </div>
    </div>
    ...
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-4">
      <div class="flex-1 w-full lg:w-auto">
        <input
          v-model="searchQuery"
          :placeholder="translations.dashboard?.publishers?.title"
          type="text"
      ...
      <div class="flex gap-3 w-full lg:w-auto">
        <button
          @click="publisherModal.openModal()"
          class="bg-gray-200 text-black px-4 py-2 rounded-lg hover:bg-gray-300 transition-colors flex-1 lg:flex-none whitespace-nowrap"
        >
          {{ translations.dashboard?.publishers?.addNew }}
        </button>
        <button
          class="bg-[var(--color-primary)] text-white px-4 py-2 rounded-lg hover:opacity-90 transition-opacity flex-1 lg:flex-none"
        >
          Export
        </button>
      </div>
    ...
    <div class="flex flex-wrap items-center justify-between gap-4 mb-4">
      <div class="flex flex-wrap items-center gap-2">
        <span class="text-sm font-medium text-gray-600 mr-2">{{
          translations.dashboard?.publishers?.sortBy
        }}</span>
        <button
      ...
      <button @click="clearFilters" class="text-sm text-blue-600 hover:underline">
        {{ translations.dashboard?.publishers?.clearFilters }}
      </button>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-6">
      <div class="flex items-center bg-yellow-50 p-4 rounded-lg">
        <div class="flex flex-col">
          <p class="text-sm font-medium text-gray-600">{{ translations.dashboard?.publishers?.stats?.total }}</p>
      ...
      <div class="flex items-center bg-green-50 p-4 rounded-lg">
        <div class="flex flex-col">
          <p class="text-sm font-medium text-gray-600">{{ translations.dashboard?.publishers?.stats?.orders }}</p>
      ...
      <div class="flex items-center bg-blue-50 p-4 rounded-lg">
        <div class="flex flex-col">
          <p class="text-sm font-medium text-gray-600">{{ translations.dashboard?.publishers?.stats?.books }}</p>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mt-6 mb-8">
        ...
        <div v-for="house in filteredPublishingHouses" :key="house.id" class="border-b border-gray-200 p-4 hover:bg-gray-50 transition-colors">
          ...
          <div class="flex flex-wrap items-center gap-x-4 gap-y-2 mt-4 pt-3 border-t">
            <button @click="router.push(`/dashboard/publishing-house/${house.id}`)" class="action-btn text-indigo-600">
              ...
              {{ translations.dashboard?.publishers?.actions.view }}
            </button>
            <button @click="openEditModal(house)" class="action-btn text-blue-600">
              ...
              {{ translations.dashboard?.publishers?.actions.edit }}
            </button>
          ...
          <thead class="bg-gray-50">
            <tr>
              <th @click="handleSort('name')" class="table-header">
                {{ translations.dashboard?.publishers?.table?.name }} {{ getSortIcon('name') }}
              </th>
              <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                {{ translations.dashboard?.publishers?.table?.id }}
              </th>
              <th @click="handleSort('orders')" class="table-header">
                {{ translations.dashboard?.publishers?.table?.orders }} {{ getSortIcon('orders') }}
              </th>
              <th @click="handleSort('status')" class="table-header">
                {{ translations.dashboard?.publishers?.table?.status }} {{ getSortIcon('status') }}
              </th>
              <th @click="handleSort('nmBook')" class="table-header">
                {{ translations.dashboard?.publishers?.table?.books }} {{ getSortIcon('nmBook') }}
              </th>
              <th
                class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"
              >
                {{ translations.dashboard?.publishers?.table?.actions }}
              </th>
            </tr>
          </thead>
            ...
            <tr v-for="house in filteredPublishingHouses" :key="house.id" class="hover:bg-gray-50 transition-colors">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                <div class="flex items-center space-x-4">
                  <RouterLink :to="`/dashboard/publishing-house/${house.id}`" class="text-indigo-600 hover:text-indigo-800 transition-colors">{{ translations.dashboard?.publishers?.actions.view }}</RouterLink>
                  <button @click="openEditModal(house)" class="text-blue-600 hover:text-blue-800 transition-colors">{{ translations.dashboard?.publishers?.actions.edit }}</button>
                  <button @click="openDeleteDialog(house)" class="text-red-600 hover:text-red-800 transition-colors">{{ translations.dashboard?.publishers?.actions.delete }}</button>
                </div>
              </td>
            </tr>
          </tbody>
      ...
      <div v-if="filteredPublishingHouses.length === 0" class="text-center py-16">
        ...
        <h3 class="mt-2 text-sm font-medium text-gray-900">{{ translations.dashboard?.publishers?.emptyHeader }}</h3>
        <p class="mt-1 text-sm text-gray-500">{{ translations.dashboard?.publishers?.emptySubtext }}</p>
        <button
          @click="clearFilters"
